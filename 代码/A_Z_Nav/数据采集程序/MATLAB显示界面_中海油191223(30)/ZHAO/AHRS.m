...
%     com_n = GetSerialCom();  %自动获取串口号
    ...
    %com_n = sprintf('COM%d',get(handles.com,'value'));
    rates = [300 600 1200 2400 4800 9600 19200 38400 43000 56000 57600 115200 230400];
%     baud_rate = rates(get(handles.rate,'value'));
    ...
    set(scom,'BaudRate',baud_rate,'Parity',jiaoyan,'DataBits',...
        data_bits,'StopBits',stop_bits,'BytesAvailableFcnCount',360,...  %%%接送数据个数，此处22字节,响应每次只读取前22字节也是一种方案，但是响应太频繁使得图像显示和保存数据的数组占用内存迅速占满！或者去4400，减少响应频次！
...
%    end
   ...
   %}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%接收传感器原始数据%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   accx_raw1 = out(12+iii*len)*256+out(11+iii*len);
   if (accx_raw1 > 32768)
       accx_raw1 = accx_raw1 - 65536;
   end   
   accy_raw1 = out(14+iii*len)*256+out(13+iii*len);
   if (accy_raw1 > 32768)
       accy_raw1 = accy_raw1 - 65536;
   end   
   accz_raw1 = out(16+iii*len)*256+out(15+iii*len);
   if (accz_raw1 > 32768)
       accz_raw1 = accz_raw1 - 65536;
   end
   accx_raw2 = out(32+iii*len)*256+out(31+iii*len);
   if (accx_raw2 > 32768)
       accx_raw2 = accx_raw1 - 65536;
   end   
   accy_raw2 = out(34+iii*len)*256+out(33+iii*len);
   if (accy_raw2 > 32768)
       accy_raw2 = accy_raw2 - 65536;
   end   
   accz_raw2 = out(36+iii*len)*256+out(35+iii*len);
   if (accz_raw2 > 32768)
       accz_raw2 = accz_raw2 - 65536;
   end
   
  accx_raw= accx_raw1*10+ accx_raw2;
  accy_raw= accy_raw1*10+ accy_raw2;
  accz_raw= accz_raw1*10+ accz_raw2;
