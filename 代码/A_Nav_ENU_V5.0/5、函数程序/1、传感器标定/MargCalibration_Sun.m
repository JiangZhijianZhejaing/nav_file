function [Gyro_Set_InitCS, Acc_Set_InitCS, Mag_Set_InitCS, Marg_Number] = MargCalibration_Sun(File_Name, Sheet_Name, Wp, P, Mb, B)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function：MARG传感器标定，输出实验室模块坐标系下的Marg数据
%
% Prototype: [Gyro_Set_InitCS, Acc_Set_InitCS, Mag_Set_InitCS, Marg_Number] = MargCalibration(File_Name, Sheet_Name, Wp, P, Mb, B)
...
%         Marg_Number - 采样数
%
% Notice：详见《储志伟.基于MARG传感器的微型航姿系统[D].中国科学技术大学,2018.》第四章，其中零偏改为开机后计算，并一定阈值内归零。
%
% By Taoran Zhao
...
%% ---------------二、传感器标定---------------
% ----------1、陀螺仪标定----------
% 标定后，前3秒静置，扣除零偏(每次启动后不变，但每次启动不是固定值)
Gyro_offset = mean(Gyro_Raw_Set(1:100,:));            % 零偏，取300个平均
Gyro_Raw_Set_0 = Gyro_Raw_Set - Gyro_offset;          % 扣除每次开机的零偏
Gyro_Threshold = max(abs(Gyro_Raw_Set_0(1:100,:)));   % 阈值计算
% 阈值内角速度归0
for i = 1 : Marg_Number
    if abs(Gyro_Raw_Set_0(i,1)) <= Gyro_Threshold(1)
        Gyro_Raw_Set_0(i,1) = 0;
    end
    if abs(Gyro_Raw_Set_0(i,2)) <= Gyro_Threshold(2)
        Gyro_Raw_Set_0(i,2) = 0;
    end
    if abs(Gyro_Raw_Set_0(i,3)) <= Gyro_Threshold(3)
        Gyro_Raw_Set_0(i,3) = 0;
    end    
end
Gyro_Raw_Set_add1 = horzcat(Gyro_Raw_Set_0, ones(Marg_Number, 1));  % 加一列1
Gyro_Set_InitCS = (Gyro_Raw_Set_add1*Wp);                           % 三轴陀螺仪实际输出值（deg/s）
% ----------2、加速度计标定----------
