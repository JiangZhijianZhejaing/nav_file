...
[Gyro_Set_InitCs, Acc_Set_InitCs, Mag_Set_InitCs, Marg_Number] = ...
    MargCalibNoZero('20230521', 'Sheet1', CalibParm.Wp, CalibParm.p, CalibParm.mb, CalibParm.R);
% ---------------三、转换东北天坐标系---------------
...
P_ksub1_A = eye(4);
P_ksub1_M = eye(4);
Q_ksub1 = (1e-7)*eye(4);                         % 状态噪声方差阵
R_Acc_ksub1 = (1e-12)*eye(3);                     % 加速度计量测噪声方差阵
R_Mag_ksub1 = (1e-10)*eye(3);                     % 磁传感器量测噪声方差阵
...
for k = 1:Marg_Number
    ...
    Z_Acc_k = Acc_Set(k,:)';
    [X_k_A, P_k_A, X_k_ksub1_A] = TEKF_EKF(PHI_k_ksub1_A, X_ksub1_A, P_ksub1_A, Q_ksub1, R_Acc_ksub1, Z_Acc_k, h_Acc_k_Fun, H_Acc_k_Fun);
    % ----------2、从单位姿态四元数中剔除航向姿态信息----------
    % -----(1)变化四元数-----
    ...
    Z_Mag_k = m_nl;
    [X_k_M, P_k_M, X_k_ksub1_M] = TEKF_EKF(PHI_k_ksub1_M, X_ksub1_M, P_ksub1_M, Q_ksub1, R_Mag_ksub1, Z_Mag_k, h_Mag_k_Fun, H_Mag_k_Fun);    
