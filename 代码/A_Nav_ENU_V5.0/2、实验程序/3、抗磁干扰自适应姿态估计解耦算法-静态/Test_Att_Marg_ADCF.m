...
% % ----------2、分配内存----------
% Eul_ADCF_Set = zeros(Marg_Number,3);             % 提前分配Eul_ADCF_Set内存 
% % ----------3、DCF初始化----------
% Qnb_ksub1 = Qnb_Init; 
% % ---------------六、DCF---------------
% for k = 1:Marg_Number
%     gb = Acc_Set(k,:)';                         % 加速度量测
%     gb_mod = sqrt(gb'*gb);                      % 加速度量测模长
%     ea = abs(gb_mod - 1)/1;                     % 加速度量测模长误差
%     xa = 1e-3;                                  % 误差阈值
%     if ea > xa
%         ea = xa;
%     end
%     u_a = 1-(1/xa)*ea;                          % 倾角调节权值
%     % ----------1、陀螺仪预测姿态四元数Qnb_k_ksub1----------
%     W_Deg_ksub1 = Gyro_Set(k,:);
%     % 一阶龙格库塔法
%     Qnb_k_ksub1 = RungekutaOne(W_Deg_ksub1, Sample_Interval, Qnb_ksub1);    % k时刻 Qnb_k_ksub1(归一)
%     % ----------2、一级互补滤波---------- 
%     % -----(1)载体加速度估计-----
%     Cbn = Qnb2Cnb(Qnb_k_ksub1)';
...
%     % -----(5)解算Q_a-----
%     Q_a = QuaMulQua(Qnb_k_ksub1,Q_ae);    
%     % ----------2、二级互补滤波----------
%     % -----(1)导航系磁矢量估计-----
