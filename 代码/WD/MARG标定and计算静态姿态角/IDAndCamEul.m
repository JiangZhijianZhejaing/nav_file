...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Gyro_Raw_Set=Marg_Raw_Set(:,1:3);                      %陀螺仪的原始数据集
    Acc_Raw_Set=Marg_Raw_Set(:,4:6);                        %加速度计原始数据集
    Mag_Raw_Set=Marg_Raw_Set(:,7:9);                      %磁传感器原始数据集
    
    %% 陀螺仪标定
    Gyro_offset=mean(Gyro_Raw_Set(1:300,:));             %零偏:一是固定误差，二是漂移
    Gyro_Raw_Set=Gyro_Raw_Set-Gyro_offset;        %因此，需要在信息融合中校正的只有漂移，可以将其误差模型等效为随机常数与白噪声之和
    ...
    Gyro_Raw_Set_add1=horzcat(Gyro_Raw_Set,ones(Marg_Number,1));      % 陀螺仪实际输出值（deg/s），依次是绕x轴角速率、绕y轴角速率、绕z轴角速率
    Gyro_Set_0=(Gyro_Raw_Set_add1*Wp);
